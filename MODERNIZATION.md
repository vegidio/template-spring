...
```json
{
    ...
    "title": "Validation failed for 'Country'",
    "status": 400,
    "detail": "Code must be 3 characters long",
    "instance": "/api/v1/countries/US",
    "field": "code",
    "rejectedValue": "US"
}
```
...
- `src/main/kotlin/io/vinicius/tplspring/exception/RestExceptionHandler.kt`
...
- Enabled `spring.threads.virtual.enabled=true` in configuration

**Benefits:**

- Dramatically improved concurrency and scalability
- Better resource utilization (thousands of threads with minimal memory overhead)
- No code changes required - works transparently
- Perfect for I/O-bound operations (database calls, REST APIs, etc.)
- Especially powerful with blocking JDBC operations

**Performance Impact:**

- Traditional threads: ~1-2MB per thread → limited to hundreds/thousands
- Virtual threads: ~1KB per thread → can handle millions of concurrent requests
